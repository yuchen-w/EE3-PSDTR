...
float *mag;
complex *C;
volatile int index = 0; 		//volatile prevents compiler from optimising
int i, j, k, n;
...
void wait_buffer_ex2(void);    
void wait_buffer_ex3(void);
void ord_dft(int, complex*);
void ord_idft(int, complex*);                  
/********************************** Main routine ************************************/
void main()
{      
...
void init_arrays(void)
{
    ...
    C 			 = (complex *) calloc(BUFLEN, sizeof(complex));	//Complex array
    mag 		 =	(float *) calloc(BUFLEN, sizeof(float)); /* FFT array */
}
...
void ord_dft(int N, complex *X)							//this is broken
{
	complex sum;										//declare this as an array
	for (k = 0; k<N; k++)
	{
		sum.r = 0;
		sum.i = 0;
		for (n = 0; n<N; n++)
		{
			sum.r += X[n].r*cos(2*PI*n*k/N) + X[n].i*sin(2*PI*n*k/N);
			sum.i += X[n].i*cos(2*PI*n*k/N) - X[n].r*sin(2*PI*n*k/N);
...
void ord_idft(int N, complex *X)						//this is broken
{
	complex sum;
	sum.r = 0;
	sum.i = 0;
